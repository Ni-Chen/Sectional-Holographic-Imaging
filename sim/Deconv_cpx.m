...
if ismember(case_type,[7, 8, 9])
    [im_ori,otf_ori,y_ori] = GenerateDataCpx('Poisson',50);
else
    [im_ori,otf_ori,y_ori] = GenerateDataCpx('Gaussian',30);
end
...
switch case_type
    ...
    case 4 %
        ...
        lamb = 1e-3; rho_admm = 0.1; tau_pd = 0.1;  rho_pd = 1.1;
...
%             R_N12 = CostMixNormSchatt1([sz,3],1); % Mixed Norm 1-Schatten (p = 1)        
    ...
    if isNonNeg
        R_POS = CostNonNeg(C.sizeout);           % Non-Negativity, Not work for complex value
    ...
    switch solv_type
        ...
        case 'ADMM'
            ...
            if ~isNonNeg % ADMM LS + TV/HS
                ...
                if  strcmp(cost_type, 'LS') % ADMM LS + TV + NonNeg
                    disp('ADMM + LS + NonNeg');
                    Fn = {lamb*R_N12, R_POS*C}; % Functionals F_n constituting the cost
                    Hn = {G*C, LinOpIdentity(size(im))}; % Associated operators H_n
        ...
        case 'PD' % PrimalDual Condat KL
            if ~isNonNeg
                ...
                if  strcmp(cost_type, 'LS') % ADMM LS + TV + NonNeg
                    ...
                    Fn = {lamb*R_N12};
                    Hn = {G*C};
                    optSolve = OptiPrimalDualCondat(Fwd,R_POS*C,Fn,Hn);
                ...
                elseif strcmp(cost_type, 'KL') % ADMM LS + TV + NonNeg
                    ...
                    Fn = {lamb*R_N12, KL};
                    Hn = {G*C,H};
                    optSolve = OptiPrimalDualCondat([],R_POS*C,Fn,Hn);
        ...
        case 'DR' % Douglas-Rachford
            ...
            gama = 10; % [0 Inf]
            lamb = 2;  % [0 2]
            optSolve = OptiDouglasRachford(Fwd, R_POS*C, [], gama, lamb);
            optSolve.OutOp=OutputOptiSNR(1,im,round(maxit/10));
