...
%% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% input_pathn = './Data/';
% img = im2double(imread([input_pathn, 'res.png']));
% g = 1-img./max(img(:));
% pixel_num = 1024;
% detector_size = 4;
% lambda = 0.532;
...
% %% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% onion
input_pathn = './Data/onion/';
img = im2double(imread([input_pathn, 'onion.bmp']));
% bg = im2double(imread([input_pathn, 'BG/BG.bmp']));
% g = (img - bg)./sqrt(bg);
g = img./max(img(:));
pixel_num = 2048;
detector_size = 3.45;
...
if view_standard_reconstruction
    ...
    figure; imagesc(abs(f_reconstruct));
    title('amp'); xlabel('X'); ylabel('Y'); axis image; axis equal;
    figure; imagesc(angle(f_reconstruct));
    title('Phase'); xlabel('X'); ylabel('Y'); axis image; axis equal;
...
cmbxz = max(intensity, [], 1);
cmbxz = rot90(flipud(squeeze(cmbxz)),-1);
imwrite(cmbxy, './Outputs/cmb_amp.tif');
imwrite(angle(f_reconstruct), './Outputs/cmb_phase.tif');
...
%     figure; imagesc(est); axis image; title('Estimated'); colorbar();
    figure; imagesc(abs(est)); axis image; title('Estimated'); colorbar();
    resid = g - est;
    figure; imagesc(abs(resid)); axis image; title('Residual'); colorbar();
end