function [x,x_debias,objective,times,debias_start,mses,max_svd] = ...
         MyTwIST(y,A,tau,Nx,Ny,Nz,varargin)
%
% Usage:
    ...
    while for_ever
        ...
        if (IST_iters >= 2) | ( TwIST_iters ~= 0)
            ...
            if f > prev_f
                ...
                % of max_svs
                max_svd = 2*max_svd;
                count_IncrementingS=count_IncrementingS+1;
                if verbose
...
%     if( 0 )
        ...
        if(Ny==1)
            figure(999);plot(real(x));colormap gray;set(gcf,'color','w');
        elseif(Nz==1)
            figure(999);imagesc(reshape(real(x),Nx,Ny)); ...
                set(gcf,'color','w');colorbar;axis image;drawnow;
        else
            figure(999);imagesc(plotdatacube(reshape(real(x),[Nx Ny Nz]),min(5,Nz)));...
                set(gcf,'color','w');colorbar;axis image;drawnow;

        end;
        title(['Iteration:',num2str(iter)]);drawnow;
    end;
