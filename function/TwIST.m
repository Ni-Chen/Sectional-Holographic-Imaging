...
TwIST is distributed under the terms of the GNU General Public License 2.0.
...
authors do not make any representation or warranty of any kind concerning the merchantability of 
...
======================================= Required inputs ============================================
 ...
 A: if y and x are both 1D vectors, A can be a k*n (where k is the size of y and n the size of x)
    ...
    In any other case (if y and/or x are 2D arrays), A has to be passed as a handle to a function 
    which computes products of the form A*x; another handle to a function AT which computes products
    of the form A'*x is also required in this case. The size of x is determined as the size of the 
    result of applying AT.
 ...
 'lambda' = lam1 parameters of the  TwIST algorithm:
            ...
            If min eigenvalue of A'*A == 0, or unknwon, set lam1 to a value much smaller than 1.
 ...
 'StopCriterion' = type of stopping criterion to use
                   0 = algorithm stops when the relative change in the number of non-zero components
                       of the estimate falls below 'ToleranceA'
 ...
 'ToleranceD' = stopping threshold for the debiasing phase:
                Default = 0.0001.
                If no debiasing takes place, this parameter, if present, is ignored.

 'MaxiterA' = maximum number of iterations allowed in the main phase of the algorithm.
              Default = 1000

 'MiniterA' = minimum number of iterations performed in the main phase of the algorithm.
              Default = 5
...
============ Outputs ==============================
  ...
  debias_start = iteration number at which the debiasing phase started. If no debiasing took place,
      ...
      for i=1:2:(length(varargin)-1)
        switch upper(varargin{i})
         ...
         case 'TRUE_X'
           compute_mse = 1;
%            true = varargin{i+1};
           true_x = varargin{i+1};   % modified by Ni Chen
            size(true_x)
            size(y)
           if prod(double((size(true_x) == size(y))))
               plot_ISNR = 1;
           end
         case 'AT'
           AT = varargin{i+1};
    ...
    %{
    if A is a matrix, we find out dimensions of y and x, and create function handles for 
    multiplication by A and A', so that the code below doesn't have to distinguish between the 
    handle/not-handle cases
    %}
    ...
    % if phi was given, check to see if it is a handle and that it accepts two arguments
...
%     if compute_mse && (size(true_x)~=size(x))  
    if compute_mse && (~isequal(size(true_x),size(x)))  % Modified by Ni Chen
       error(['Initial x has incompatible size']); 
    end

    % if tau is large enough, in the case of phi = l1, thus psi = soft, the optimal solution is the 
    % zero vector
    if phi_l1
       max_tau = max(abs(Aty(:)));
    ...
    while cont_outer
        ...
        while for_ever
            ...
            if (IST_iters >= 2) || ( TwIST_iters ~= 0)
                ...
                if f > prev_f
                    % if monotonicity  fails here  is  because max eig (A'A) > 1. Thus, we increase 
                    % our guess of max_svs
                    max_svd = 2*max_svd;
                    if verbose
        ...
        switch stopCriterion
            case 0
                % compute the stopping criterion based on the change of the number of non-zero 
                % components of the estimate
                criterion =  num_changes_active;
            case 1
                % compute the stopping criterion based on the relative variation of the objective
                % function.
        ...
        if verbose
            if plot_ISNR
                fprintf(1,'Iteration=%4d, ISNR=%4.5e  objective=%9.5e, nz=%7d, criterion=%7.3e\n',...
                    iter, 10*log10(sum((y(:)-true_x(:)).^2)/sum((x(:)-true_x(:)).^2) ), ...
    ...
    %--------------------------------------------------------------
    % If the 'Debias' option is set to 1, we try to remove the bias from the l1 penalty, by applying
    % CG to the least-squares problem obtained by omitting the l1 term and fixing the zero
    ...
    if debias
        ...
        while cont_debias_cg
            ...
            % in the debiasing CG phase, always use convergence criterion based on the residual 
            % (this is standard for CG)
            if verbose
